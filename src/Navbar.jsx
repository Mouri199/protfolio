
import { motion } from "framer-motion";
import { Link } from "react-scroll";

import logo from './assets/Name Logo.png';



const Navbar = () => {

 
 

  // Define motion variants for the links
  const linkVariants = {
    hover: {
      scale: 1.1,
      transition: { duration: 0.2 },
    },
  };

  return (
    < div className="relative">
      <div className=" lg:block fixed bg-[#176B87] rounded-xl ml-10 z-50 h-[700px] w-[350px] hidden text-white mt-24 py-2">
        <div className=" text-[#FFFFFF] flex my-16 flex-col items-center justify-center gap-10">

          <img className="rounded-full w-[120px]" src={logo} alt="" />
          <Link activeClass="active" smooth={true} to="home">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              Home
            </motion.p>
          </Link>
          <Link activeClass="active" smooth={true} to="about">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              About
            </motion.p>
          </Link>
          <Link activeClass="active" smooth={true} to="services">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              Services
            </motion.p>
          </Link>
          <Link activeClass="active" smooth={true} to="project">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              Projects
            </motion.p>
          </Link>
          <Link activeClass="active" smooth={true} to="education">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              Education
            </motion.p>
          </Link>
          <Link activeClass="active" smooth={true} to="contact">
            <motion.p
              className="text-2xl cursor-pointer"
              variants={linkVariants}
              whileHover="hover"
            >
              Contact
            </motion.p>
          </Link>
          {/* <div className="text-end lg:block hidden mr-20">
            <label className="swap swap-rotate">

             
              <input type="checkbox" onClick={changeTheme} />

      
              <svg className="swap-on fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" /></svg>

           
              <svg className="swap-off fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" /></svg>

            </label>
          </div> */}

        </div>
        {/* <div className="flex justify-center gap-7">
          <motion.p
            className="text-4xl"
            variants={iconVariants}
            whileHover="hover"
          >
            <FaFacebookF />
          </motion.p>
          <motion.p
            className="text-4xl"
            variants={iconVariants}
            whileHover="hover"
          >
            <FaLinkedinIn />
          </motion.p>
          <motion.p
            className="text-4xl"
            variants={iconVariants}
            whileHover="hover"
          >
            <FaGithub />
          </motion.p>
        </div> */}
      </div>

      <div className="drawer absolute lg:hidden">
        <input id="my-drawer-2" type="checkbox" className="drawer-toggle" />
        <div className="drawer-content flex flex-col items-center justify-center">
          <label htmlFor="my-drawer-2" className="btn btn-ghost drawer-button text-4xl lg:hidden">

            <i className="fa-solid fa-bars"></i>
          </label>
        </div>
        <div className="drawer-side z-50">
          <label htmlFor="my-drawer-2" aria-label="close sidebar" className="drawer-overlay"></label>
          <ul className="menu p-4 w-80 min-h-full bg-[#176B87] text-white gap-3">
            <Link smooth={true} to="home">
              <motion.p
                className="text-2xl cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                Home
              </motion.p>
            </Link>
            <Link smooth={true} to="about">
              <motion.p
                className="text-2xl cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                About
              </motion.p>
            </Link>
            <Link smooth={true} to="services">
              <motion.p
                className="text-2xl cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                Services
              </motion.p>
            </Link>
            <Link smooth={true} to="project">
              <motion.p
                className="text-2xl cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                Projects
              </motion.p>
            </Link>
            <Link smooth={true} to="education">
              <motion.p
                className="text-2xl cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                Education
              </motion.p>
            </Link>
            <Link smooth={true} to="contact">
              <motion.p
                className="text-2xl mb-12 cursor-pointer"
                variants={linkVariants}
                whileHover="hover"
              >
                Contact
              </motion.p>
            </Link>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
